...
export function checkTimestamps(model: ModelCtor<Model<any, any>>) {
  ...
  if (!keys.includes(createdAtName)) {
    console.log(`[${"WARN".yellow} timestamps] Model '${model.name}' doesn't contain '${createdAtName}'.`);
  }
  if (!keys.includes(updatedAtName)) {
    console.log(`[${"WARN".yellow} timestamps] Model '${model.name}' doesn't contain '${updatedAtName}'.`);
  }
}
